...
import { skillItems, dateRange, formatMonthYear } from "@/components/templates/helpers";
...
export async function buildResumeDocx(resume) {
  ...
  if (s.certifications?.length) {
    ...
    s.certifications.forEach((c) =>
      children.push(para([c.name, c.issuer, formatMonthYear(c.date)].filter(Boolean).join(" — ")))
    );
  }
