...
export async function extendUserSubscription({ userId, plan, razorpayOrderId, razorpayPaymentId, mongooseSession }) {
  ...
  if (existing && existing.expiryDate > now) {
    ...
    // The expiry just moved out — any reminder already sent was for the old
    // date, so clear it and let the cron send a fresh one nearer this one.
    existing.expiryReminderSentAt = null;
