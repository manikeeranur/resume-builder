  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      if (isNewUser) {
        ...
        // (serverless can freeze this function right after it resolves,
        // cutting off an unawaited send before the SMTP handshake finishes).
        await sendWelcomeEmail({ to: user.email.toLowerCase(), name: user.name }).catch(() => {});
      }
