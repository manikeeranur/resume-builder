...
const PAYMENT_METHOD_LABELS = { card: "Card", upi: "UPI", netbanking: "Net banking", wallet: "Wallet", emi: "EMI" };
function paymentMethodLabel(method) {
  return (method && PAYMENT_METHOD_LABELS[method.toLowerCase()]) || "Online payment";
}
...
export async function getInvoiceData(paymentId, { userId, isAdmin }) {
  ...
  const companyName = process.env.INVOICE_COMPANY_NAME || "ResumePro";
  ...
  return {
    ...
    subscription: subscription ? { startDate: subscription.startDate, expiryDate: subscription.expiryDate } : null,
    company: { name: companyName, supportEmail },
    seller: { name: companyName, email: supportEmail, gstin: process.env.INVOICE_GSTIN || "Not provided" },
    taxDetails: process.env.INVOICE_TAX_DETAILS || "Not provided",
  };
}
