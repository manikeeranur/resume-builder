...
// Templates 3, 4, 5 & 6 have full-bleed backgrounds (a header band, a
// sidebar, a decorative page image) designed to reach the page edge, unlike
// Templates 1/2 which have no padding of their own and rely on this margin
// for breathing room — so only the full-bleed templates skip it.
const FULL_BLEED_TEMPLATES = new Set(["template-3", "template-4", "template-5", "template-6"]);

// Renders one resume to PDF bytes on an existing browser instance (a new tab
// per call, not a new browser) so callers that need many resumes — e.g. a
// dashboard's preview batch — can reuse a single Chromium process instead of
// launching one per resume.
export async function renderResumePdf(browser, { resumeId, templateId, origin, req, pageRanges }) {
  ...
  try {
    ...
    await page.goto(`${origin}/resumes/${resumeId}/print`, {
    ...
    const margin = FULL_BLEED_TEMPLATES.has(templateId)
