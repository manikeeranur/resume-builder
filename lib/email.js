...
import { renderPaymentReceivedEmail, renderPlanActiveEmail, renderWelcomeEmail } from "@/lib/emailTemplates";
...
async function sendMail({ to, subject, html, attachments }) {
  ...
  try {
    await t.sendMail({ from: `"${fromName}" <${process.env.SMTP_FROM}>`, to, subject, html, attachments });
  } catch (err) {
...
export async function sendPaymentSuccessEmail({ to, name, planName, amount, currency, razorpayPaymentId, attachments }) {
  const companyName = process.env.EMAIL_FROM_NAME || "ResumePro";
  const html = renderPaymentReceivedEmail({
    name,
    planName,
    amount,
    currency,
    razorpayPaymentId,
    paidAt: new Date(),
    accountUrl: `${appOrigin()}/account/subscription`,
    companyName,
    supportEmail: process.env.SMTP_FROM,
  });
  await sendMail({ to, subject: "Payment received", html, attachments });
}
...
export async function sendSubscriptionActivatedEmail({ to, name, planName, expiryDate }) {
  const companyName = process.env.EMAIL_FROM_NAME || "ResumePro";
  const html = renderPlanActiveEmail({
    name,
    planName,
    expiryDate,
  ...
  await sendMail({ to, subject: `Your ${planName} plan is now active`, html });
}
...
export async function sendResubscribeReminderEmail({ to, name, planName }) {
  await sendMail({
    to,
    subject: "We miss you — come back to premium",
    html: `<p>Hi ${name},</p><p>Your ${planName} plan has lapsed. Resubscribe anytime from the pricing page to unlock premium templates, unlimited downloads, and more.</p>`,
  });
}
...
export async function sendWelcomeEmail({ to, name }) {
  ...
  const html = renderWelcomeEmail({
    name,
    createResumeUrl: `${appOrigin()}/resumes`,
    upgradeUrl: `${appOrigin()}/pricing`,
    companyName,
    supportEmail: process.env.SMTP_FROM,
  });
  await sendMail({ to, subject: "Welcome to ResumePro", html });
}
