...
export async function getMergedTemplateList() {
  await dbConnect();
  const [dynamicDocs, overrides] = await Promise.all([
    Template.find({ active: true }).sort({ createdAt: 1 }),
    TemplateOverride.find(),
  ]);
  const overrideMap = Object.fromEntries(overrides.map((o) => [o.templateId, o.category || null]));
  const builtins = TEMPLATE_LIST.map((t) =>
    t.id in overrideMap ? { ...t, category: overrideMap[t.id] } : t
  );
  return [...builtins, ...dynamicDocs.map(dynamicTemplateToMeta)];
}
